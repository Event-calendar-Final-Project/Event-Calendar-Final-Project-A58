...
export async function updateUserPhoto(file, handle, oldPhotoUrl) {
  ...
  export async function addEventPhoto(file, event) {

    const storage = getStorage();
    const storageRef = ref(storage, `events/${event}/${file.name}`);
  ...
  export async function updateEventPhoto(file, event, oldPhotoUrl) {
    const storage = getStorage();
    ...
    if (oldPhotoUrl) {
        ...
        const oldPhotoName = pathParts[pathParts.length - 1];
        console.log('Old photo name:', oldPhotoName);
        const oldPhotoRef = ref(storage, `events/${event}/${oldPhotoName}`);
        await deleteObject(oldPhotoRef);
    }
  
    const newPhotoRef = ref(storage, `events/${event}/${file.name}`);
    const uploadTask = uploadBytesResumable(newPhotoRef, file);
